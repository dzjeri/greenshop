...
const CheckoutModal = ({ orderInfo, products, isOpen, closeModal }) => {
  const subtotal = products.reduce((sum, cur) => sum + cur.quantity * cur.fullPrice, 0);
  const discount = 0;
      ...
      <div className={classes.content}>
        ...
        <div className={classes.body}>
                  ...
                  <div className={classes.productCard}>
                    <Link className={classes.product} to={`/shop/${p.slug}`}>
                      <div className={classes.productPhotoWrapper}>
                        <img className={classes.productPhoto} src={p.src} alt="Product Photo" />
                      </div>
                    ...
                    <div className={classes.quantity}>{'(x ' + p.quantity + ')'}</div>
                    <div className={classes.subtotal}>{formatting.formatPrice(p.quantity * p.fullPrice)}</div>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className={classes.footer}>
            ...
            <button onClick={closeModal} className={classes.trackButton}>Track your order</button>
          </div>
        </div>
      </div>
    </div>
